...
function ready(error, data) {
  ...
  d3.select("#perCategory").on('change', update);
  d3.select("#perCheckbox").on('change', update); // Sort checkbox
  ...
  function update() {
    ...
    catInt = d3.select("#perCategory").property('value');

    keys = data.columns.slice(1, 3); //Filter columns for Group Labels
